...
public  class ConnectionUpdate : MonoBehaviour
{
    ...
    public  void ConnectionUp(Transform t1, Transform t2)
    {
        firstObj = t1;
        secontObj = t2;
    }
    private void FixedUpdate()
    {
        ...
        if (firstObj != null && secontObj != null)
        {
            ...
            transform.localScale = new Vector3(connectionSize, connectionSize, Vector3.Distance(v1, v2));
            transform.position = new Vector3((v1.x + v2.x) / 2, (v1.y + v2.y) / 2, (v1.z + v2.z) / 2);
            transform.rotation = Quaternion.RotateTowards(Quaternion.LookRotation(v1), Quaternion.LookRotation((v1 - v2)), 360f);
        }
        else{ DeleteConnection();}
    }
